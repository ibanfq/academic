<?php 
	...
	if ($students === false) {
		$html->addCrumb($event['Activity']['name'], Environment::getBaseUrl() . "/attendance_registers/edit_student_attendance/{$event['Event']['id']}"); 
	} else {
		$html->addCrumb($ar['Activity']['name'], Environment::getBaseUrl() . "/attendance_registers/edit_student_attendance/{$ar['Event']['id']}"); 
	}
?>
...
<?php if ($students === false): // Edition is blocked, show subject information ?>
	...
	<?php echo $form->create('AttendanceRegister', array('url' => $html->url(null))); ?>
	<fieldset>
		...
		<legend>Estudiantes</legend>
			...
			<?php $checked = empty($ar['AttendanceRegister']['secret_code'])? 'checked' : ''; ?>
						<tr id="row_<?php echo $i?>">
							<td onclick="toogleCheckBox(<?php echo $student['id'] ?>)"><?php echo "{$student['first_name']} {$student['last_name']}"?></td>
